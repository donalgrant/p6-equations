...
class Solutions {
  ...
  multi method new( @rpn_str ) { self.new( S=>@rpn_str.map({ RPN.new($_) }).map({ (~$_,+$_) }) ) }
  ...
  multi method save(%rpn) { for %rpn.keys -> $rpn { self.save($rpn) } }
  multi method save(@rpn) { for @rpn.keys -> $rpn { self.save($rpn) } }
  ...
  method list     { %!S<>:k }
  method values   { %!S<>:v.Set }
  method rpn_list { self.list.map({ RPN.new($_) }) }
  method found    { self.elems > 0 }

  method valid_for( $goal ) { self.list.grep({ %.S{$_}==$goal }) }
  method formable( Bag $all, Bag $req ) { self.rpn_list.grep({ ($_.Bag (<=) $all)           and ($_.Bag (>=) $req) }) }
  method one-away( Bag $all, Bag $req ) { self.rpn_list.grep({ ($_.Bag  (-) $all).total==1  and ($_.Bag (>=) $req) }) }
